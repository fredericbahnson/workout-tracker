...
// Slider thumb styles for cross-browser/iOS compatibility
const sliderStyles = `
  .rfem-slider::-webkit-slider-thumb {
  ...
  .rfem-slider::-webkit-slider-runnable-track {
    height: 8px;
        ...
        {interactive && (
          <div className="mb-4 px-2">
            <input
              ...
              onInput={e => setMaxReps(Number((e.target as HTMLInputElement).value))}
              className="rfem-slider w-full h-8 bg-transparent cursor-pointer touch-pan-x"
              style={{
                WebkitAppearance: 'none',
                appearance: 'none',
                background: `linear-gradient(to right, #10b981 0%, #10b981 ${((maxReps - 5) / 25) * 100}%, #e5e7eb ${((maxReps - 5) / 25) * 100}%, #e5e7eb 100%)`,
                borderRadius: '9999px',
                height: '8px',
              }}
            />
