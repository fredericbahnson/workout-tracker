...
export function ReviewStep({
  ...
  const totalExercises = new Set(groups.flatMap(g => g.exerciseAssignments.map(a => a.exerciseId)))
    .size;
  ...
  const categorizedExercises = isMixedMode
    ? (() => {
        const rfemExercises: {
          exercise: NonNullable<ReturnType<typeof exerciseMap.get>>;
          group: (typeof groups)[0];
        }[] = [];
        const simpleExercises: {
          exercise: NonNullable<ReturnType<typeof exerciseMap.get>>;
          assignment: (typeof groups)[0]['exerciseAssignments'][0];
          group: (typeof groups)[0];
        }[] = [];
  ...
  return (
    <div className="space-y-4">
      ...
      {isSimpleMode && (
        <Card className="p-4">
          <h3 className="font-medium text-gray-900 dark:text-gray-100 mb-3">Exercise Targets</h3>
                ...
                <div className="space-y-1">
                  {group.exerciseAssignments.map(a => {
                    ...
                    const progressionType = isTimeBased
                      ? a.simpleTimeProgressionType
                      : a.simpleRepProgressionType;
                    ...
                    return (
                      <div
                        key={a.exerciseId}
                        className="flex flex-wrap items-center gap-x-2 gap-y-0 text-sm pl-2"
                      >
                        ...
                        {progressionType && progressionType !== 'constant' && increment && (
                          <span className="text-gray-500 dark:text-gray-400">
                            (+{increment} {isTimeBased ? 'sec' : 'reps'}{' '}
                            {PROGRESSION_INTERVAL_LABELS[progressionType].toLowerCase()})
                          </span>
                        )}
                        {isWeighted &&
                          ...
                          weightIncrement && (
                            <span className="text-gray-500 dark:text-gray-400">
                              ({formatWeightIncrement(weightIncrement)}{' '}
                              {PROGRESSION_INTERVAL_LABELS[weightProgressionType].toLowerCase()})
                            </span>
                          )}
                      </div>
                    );
        ...
        <Card className="p-4">
          ...
          <div className="space-y-4">
            ...
            {categorizedExercises.rfemExercises.length > 0 && (
              <div>
                <div className="flex items-center gap-2 mb-2">
                  <Badge variant="push" className="text-2xs">
                    RFEM
                  </Badge>
            ...
            {categorizedExercises.simpleExercises.length > 0 && (
              <div>
                <Badge variant="other" className="text-2xs mb-2">
                  Simple Progression
                </Badge>
                <div className="pl-2 space-y-1">
                  {categorizedExercises.simpleExercises.map(({ exercise, assignment }) => {
                    ...
                    const baseValue = isTimeBased
                      ? assignment.simpleBaseTime
                      : assignment.simpleBaseReps;
                    const progressionType = isTimeBased
                      ? assignment.simpleTimeProgressionType
                      : assignment.simpleRepProgressionType;
                    const increment = isTimeBased
                      ? assignment.simpleTimeIncrement
                      : assignment.simpleRepIncrement;
                    ...
                    return (
                      <div
                        key={exercise.id}
                        className="flex flex-wrap items-center gap-x-2 gap-y-0 text-sm"
                      >
                        <span className="text-gray-600 dark:text-gray-400">{exercise.name}:</span>
                        <span className="font-medium">
                          {baseValue || '?'} {isTimeBased ? 'sec' : 'reps'}
                        ...
                        {isWeighted &&
                          ...
                          weightIncrement && (
                            <span className="text-gray-500 dark:text-gray-400">
                              (+{weightIncrement} {getWeightUnitLabel()}/
                              {weightProgressionType === 'per_workout' ? 'workout' : 'week'})
                            </span>
                          )}
                      </div>
                    );
            ...
            {categorizedExercises.conditioningExercises.length > 0 && (
              <div>
                <Badge variant="core" className="text-2xs mb-2">
                  Conditioning
                </Badge>
                <div className="pl-2 space-y-1">
                  {categorizedExercises.conditioningExercises.map(({ exercise, assignment }) => {
                    ...
                    const increment = isTimeBased
                      ? assignment.conditioningTimeIncrement
                      : assignment.conditioningRepIncrement;
      ...
      {/* Rotation Preview */}
      <Card className="p-4">
        <h3 className="font-medium text-gray-900 dark:text-gray-100 mb-3">First Week Preview</h3>
