...
const CardComponent = forwardRef<HTMLDivElement, CardProps>(
  ({ className = '', variant = 'default', children, ...props }, ref) => {
    const variants = {
      ...
      interactive:
        'bg-white dark:bg-dark-surface border border-gray-200 dark:border-dark-border hover:border-gray-300 dark:hover:border-dark-border-hover hover:shadow-md cursor-pointer transition-all',
    };

    return (
      <div ref={ref} className={`rounded-xl ${variants[variant]} ${className}`} {...props}>
        {children}
      </div>
    );
  }
...
const CardHeaderComponent = forwardRef<HTMLDivElement, HTMLAttributes<HTMLDivElement>>(
  ({ className = '', children, ...props }, ref) => (
    <div
      ref={ref}
      className={`px-4 py-3 border-b border-gray-200 dark:border-dark-border ${className}`}
      {...props}
    >
      {children}
    </div>
...
const CardFooterComponent = forwardRef<HTMLDivElement, HTMLAttributes<HTMLDivElement>>(
  ({ className = '', children, ...props }, ref) => (
    <div
      ref={ref}
      className={`px-4 py-3 border-t border-gray-200 dark:border-dark-border ${className}`}
      {...props}
    >
      {children}
    </div>
