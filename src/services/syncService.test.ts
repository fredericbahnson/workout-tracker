...
describe('SyncService', () => {
  ...
  describe('data conversion', () => {
    describe('remoteToLocalExercise', () => {
      it('converts remote exercise format to local format', () => {
        ...
        const local = SyncService.remoteToLocalExercise(remote);
    ...
    describe('localToRemoteExercise', () => {
      it('converts local exercise format to remote format', () => {
        ...
        const remote = SyncService.localToRemoteExercise(local, 'user-123');
    ...
    describe('remoteToLocalMaxRecord', () => {
      it('converts remote max record to local format', () => {
        ...
        const local = SyncService.remoteToLocalMaxRecord(remote);
    ...
    describe('localToRemoteMaxRecord', () => {
      it('converts local max record to remote format', () => {
        ...
        const remote = SyncService.localToRemoteMaxRecord(local, 'user-456');
  ...
  describe('processQueue', () => {
    ...
    it('increments retry count on failure', async () => {
      ...
      expect(mockDbSyncQueue.update).toHaveBeenCalledWith('queue-1', { retryCount: 3 });
    });
...
describe('SyncService data integrity', () => {
  describe('exercise round-trip', () => {
    it('preserves all exercise fields through conversion', () => {
      ...
      const remote = SyncService.localToRemoteExercise(original, 'user-1');
      const roundTripped = SyncService.remoteToLocalExercise(remote);
  ...
  describe('max record round-trip', () => {
    it('preserves all max record fields through conversion', () => {
      ...
      const remote = SyncService.localToRemoteMaxRecord(original, 'user-1');
      const roundTripped = SyncService.remoteToLocalMaxRecord(remote);
