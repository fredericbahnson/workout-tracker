...
export function SyncedPreferencesProvider({ children }: { children: ReactNode }) {
  ...
  const saveAndSync = useCallback(async (updated: UserPreferences) => {
    try {
  ...
  const setDefaultConditioningReps = useCallback(async (value: number) => {
    const updated = await UserPreferencesRepo.setDefaultConditioningReps(value);
    await saveAndSync(updated);
  }, [saveAndSync]);

  const setConditioningWeeklyIncrement = useCallback(async (value: number) => {
    const updated = await UserPreferencesRepo.setConditioningWeeklyIncrement(value);
    await saveAndSync(updated);
  }, [saveAndSync]);

  const setWeeklySetGoal = useCallback(async (type: ExerciseType, value: number) => {
    const updated = await UserPreferencesRepo.setWeeklySetGoal(type, value);
    await saveAndSync(updated);
  }, [saveAndSync]);

  const setRestTimer = useCallback(async (settings: Partial<TimerSettings>) => {
    const updated = await UserPreferencesRepo.setRestTimer(settings);
    await saveAndSync(updated);
  }, [saveAndSync]);

  const setMaxTestRestTimer = useCallback(async (settings: Partial<TimerSettings>) => {
